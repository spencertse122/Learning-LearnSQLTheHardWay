...
words=[x.strip().capitalize() for x in open("names.txt").readlines()]
breeds = ["Dog", "Cat", "Fish", "Unicorn"]

pet_names = words[:int(round(len(words)/2))]
owner_names = words[int(round(len(words)/2)):]
...
# and ensurtes some unowned pets and owners without pets
for i in range(0, 75):
